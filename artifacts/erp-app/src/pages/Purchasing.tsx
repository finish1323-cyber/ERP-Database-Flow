...
function SuppliersTab() {
  ...
  const openEdit = (item: any) => {
    setEditingItem(item);
    setFormData({ name: item.name, contactName: item.contactName || "", phone: item.phone || "", email: item.email || "", address: item.address || "" });
    setEditOpen(true);
  };
  ...
  return (
    <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
      <div className="flex justify-between mb-6">
        ...
        <Dialog open={addOpen} onOpenChange={(o) => {setAddOpen(o); setFormData({name:"", contactName:"", phone:"", email:"", address:""})}}>
              ...
              <TableRow key={s.id}>
                ...
                <TableCell className="text-left">
                  <div className="flex justify-end gap-2">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => confirm("تأكيد الحذف؟") && deleteMut.mutate({ id: s.id })}><Trash2 className="w-4 h-4 text-red-600" /></Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
...
function ItemsTab() {
  ...
  const [addOpen, setAddOpen] = useState(false);
  const [formData, setFormData] = useState({ name: "", category: "", description: "", defaultPrice: 0 });
  ...
  return (
    <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
      <div className="flex justify-between mb-6">
        ...
        <Dialog open={addOpen} onOpenChange={(o) => {setAddOpen(o); setFormData({name:"", category:"", description:"", defaultPrice:0})}}>
          ...
          <DialogContent dir="rtl">
            ...
            <form onSubmit={handleCreate} className="space-y-4 pt-4">
              ...
              <div className="space-y-2"><label className="text-sm">السعر الافتراضي</label><Input type="number" step="0.01" value={formData.defaultPrice} onChange={e => setFormData({...formData, defaultPrice: parseFloat(e.target.value)})} /></div>
              <Button type="submit" disabled={createMut.isPending} className="w-full">حفظ</Button>
            </form>
          </DialogContent>
        </Dialog>
            ...
            {isLoading ? <TableRow><TableCell colSpan={4} className="text-center py-8">جاري التحميل...</TableCell></TableRow> : 
             items?.map(item => (
              <TableRow key={item.id}>
                ...
                <TableCell className="text-left">
                    <Button variant="ghost" size="icon" onClick={() => confirm("حذف؟") && deleteMut.mutate({ id: item.id })}><Trash2 className="w-4 h-4 text-red-600" /></Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
...
function PriceComparisonsTab() {
  ...
  const [addOpen, setAddOpen] = useState(false);
  const [formData, setFormData] = useState({ itemId: 0, supplierId: 0, quotedPrice: 0 });
        ...
        <Dialog open={addOpen} onOpenChange={setAddOpen}>
          ...
          <DialogContent dir="rtl">
            ...
            <form onSubmit={handleCreate} className="space-y-4 pt-4">
              ...
              <div className="space-y-2"><label className="text-sm">السعر المعروض *</label><Input type="number" step="0.01" required value={formData.quotedPrice} onChange={e => setFormData({...formData, quotedPrice: parseFloat(e.target.value)})} /></div>
              <Button type="submit" disabled={createMut.isPending} className="w-full">حفظ</Button>
            </form>
          </DialogContent>
        </Dialog>
