...
import {
  ...
  useListItems,
  type Customer, type Order, type UpdateOrderStatus,
...
type CustomerForm = { name: string; phone: string; email: string; deliverySuccessRate: number };
const emptyCustomerForm: CustomerForm = { name: "", phone: "", email: "", deliverySuccessRate: 100 };
...
function CustomersTab() {
  ...
  const updateMut = useUpdateCustomer({ mutation: { onSuccess: () => { queryClient.invalidateQueries({ queryKey: getListCustomersQueryKey() }); toast({ title: "تم التعديل" }); setEditOpen(false); } } });
  ...
  const openEdit = (c: Customer) => {
    ...
    setFormData({
      ...
      phone: c.phone ?? "",
      email: c.email ?? "",
      deliverySuccessRate: typeof c.deliverySuccessRate === "string" ? parseFloat(c.deliverySuccessRate) : (c.deliverySuccessRate ?? 100),
    });
    setEditOpen(true);
  };
  ...
  return (
    <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
      <div className="flex justify-between mb-6">
        ...
        <Dialog open={addOpen} onOpenChange={(o) => { setAddOpen(o); setFormData(emptyCustomerForm); }}>
          ...
          <DialogContent dir="rtl">
            ...
            <form onSubmit={handleCreate} className="space-y-4 pt-4">
              ...
              <div className="space-y-2"><label className="text-sm">نسبة نجاح الاستلام (%)</label><Input type="number" min="0" max="100" value={formData.deliverySuccessRate} onChange={e => setFormData({ ...formData, deliverySuccessRate: parseInt(e.target.value) || 0 })} /></div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? <TableRow><TableCell colSpan={5} className="text-center py-8">جاري التحميل...</TableCell></TableRow> :
              customers?.map(c => {
                const rate = typeof c.deliverySuccessRate === "string" ? parseFloat(c.deliverySuccessRate) : c.deliverySuccessRate;
                return (
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      <Badge variant="outline" className={rate != null && rate < 50 ? "text-red-600 border-red-200 bg-red-50" : "text-green-600 border-green-200 bg-green-50"}>
...
type OrderForm = { customerId: number; items: Array<{ itemId: number; quantity: number; unitPrice: number }> };
const emptyOrderForm: OrderForm = { customerId: 0, items: [{ itemId: 0, quantity: 1, unitPrice: 0 }] };
...
function OrdersTab() {
  ...
  const [statusVal, setStatusVal] = useState<UpdateOrderStatus>("pending");
  ...
  const deleteMut = useDeleteOrder({ mutation: { onSuccess: () => { queryClient.invalidateQueries({ queryKey: getListOrdersQueryKey() }); toast({ title: "تم الحذف" }); } } });
  ...
  return (
    <div className="bg-card rounded-2xl shadow-sm border border-border p-6">
      <div className="flex justify-between mb-6">
        ...
        <Dialog open={addOpen} onOpenChange={(o) => { setAddOpen(o); setFormData(emptyOrderForm); }}>
            ...
            <form onSubmit={handleCreate} className="space-y-4 pt-4">
              ...
              <div className="space-y-2 p-4 bg-muted/30 rounded-xl border border-border">
                ...
                <Select value={formData.items[0].itemId.toString()} onValueChange={v => {
                  ...
                  const price = typeof item?.defaultPrice === "string" ? parseFloat(item.defaultPrice) : (item?.defaultPrice ?? 0);
                  setFormData({ ...formData, items: [{ ...formData.items[0], itemId: parseInt(v), unitPrice: price }] });
                }}>
                ...
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <div className="space-y-2"><label className="text-sm text-muted-foreground">الكمية</label><Input type="number" min="1" value={formData.items[0].quantity} onChange={e => setFormData({ ...formData, items: [{ ...formData.items[0], quantity: parseInt(e.target.value) || 1 }] })} /></div>
                  <div className="space-y-2"><label className="text-sm text-muted-foreground">سعر الوحدة</label><Input type="number" step="0.01" value={formData.items[0].unitPrice} onChange={e => setFormData({ ...formData, items: [{ ...formData.items[0], unitPrice: parseFloat(e.target.value) || 0 }] })} /></div>
                </div>
              </div>
            ...
            {isLoading ? <TableRow><TableCell colSpan={6} className="text-center py-8">جاري التحميل...</TableCell></TableRow> :
              orders?.map(o => (
                <TableRow key={o.id}>
                  ...
                  <TableCell className="text-left">
                    <div className="flex justify-end gap-2">
                      <Button variant="outline" size="sm" onClick={() => { setEditingOrder(o); setStatusVal(o.status as UpdateOrderStatus); setEditOpen(true); }}>تحديث الحالة</Button>
                      <Button variant="ghost" size="icon" onClick={() => { if (confirm("حذف؟")) deleteMut.mutate({ id: o.id }); }}><Trash2 className="w-4 h-4 text-red-600" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent dir="rtl">
          ...
          <form onSubmit={handleStatusUpdate} className="space-y-4 pt-4">
            <Select value={statusVal} onValueChange={(v: UpdateOrderStatus) => setStatusVal(v)}>
